...
# function)
trainImages = np.array(trainImages) ## Image shape = (28,28,1)
# print(trainImages.shape)
trainImages = (trainImages.astype("float") - 127.5) / 127.5

# build the generator
print("[INFO] building generator...")
gen = DCGAN.build_generator(7, 64, channels=3)
# print(gen.summary())
# build the discriminator
print("[INFO] building discriminator...")
disc = DCGAN.build_discriminator(28, 28, 3)
...
benchmarkNoise = np.random.uniform(-1, 1, size=(256, 100))  ## 256 - Images, 100 - single in Image size
# loop over the epochs
for epoch in range(0, NUM_EPOCHS):  ## epochs - 50
    # show epoch information and compute the number of batches per
    # epoch
    print("[INFO] starting epoch {} of {}...".format(epoch + 1, NUM_EPOCHS))
    batchesPerEpoch = int(trainImages.shape[0] / BATCH_SIZE)  ## Eg ->  trainImages.shape[0]=10,batch_size = 2
    ...
    for i in range(0, batchesPerEpoch):
        ...
        imageBatch = trainImages[i * BATCH_SIZE:(i + 1) * BATCH_SIZE]  ## imageBatch = trainImages[0*2:1*2]
        noise = np.random.uniform(-1, 1, size=(BATCH_SIZE, 100))  ## 2 images of noise vector 100
        ...
        # the data
        X = np.concatenate((imageBatch, genImages)) ## [Real Real Fake Fake]
        y = ([1] * BATCH_SIZE) + ([0] * BATCH_SIZE) ## [1,1][0,0]
        y = np.reshape(y, (-1, )) ## [1 1 0 0]
        (X, y) = shuffle(X, y) ## [1 0 1 0]
        ...
        noise = np.random.uniform(-1, 1, (BATCH_SIZE, 100))  ## 2 images of noise vector 100 [fake,fake]
        fakeLabels = [1] * BATCH_SIZE ## [1,1]
        fakeLabels = np.reshape(fakeLabels, (-1, )) ## [1 1]
        ...
        # batch for the epoch
            ...
            if epoch < 10 and i % 25 == 0:
                p = [
                    args["output"], "epoch_{}_step_{}.png".format(
                        str(epoch + 1).zfill(4),
                        str(i).zfill(5))
                ]
...
# generator model on our benchmark data
    if p is not None:
        ...
        print("[INFO] Step {}_{}: discriminator_loss={:.6f}, "
              "adversarial_loss={:.6f}".format(epoch + 1, i, discLoss,
                                               ganLoss))
        # make predictions on the benchmark noise, scale it back
        # to the range [0, 255], and generate the montage
        images = gen.predict(benchmarkNoise)  ## Create a image using noise with the prediction
